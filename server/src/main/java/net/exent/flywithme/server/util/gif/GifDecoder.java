...
/*
 * NOTE!
 * This is shamelessly modified to not use java.awt.* classes as it's
 * intended to be used with Google App Engine where these classes are not
 * whitelisted.
 */
/**
 * <p>
...
public final class GifDecoder {
    ...
    public final class GifImage {
        ...
        private final void drawFrame(final GifFrame fr) {
            ...
            final int[] px = new int[numPixels];
            System.arraycopy(pixels, 0, px, 0, numPixels);
            int sourceOffset = 0;
            int targetOffset = width * fr.top + fr.left;
            for (int y = 0; y < fr.height; ++y) {
                System.arraycopy(px, sourceOffset, img, targetOffset, fr.width);
                sourceOffset += fr.width;
                targetOffset += width + (width - fr.width);
            }
        }
         ...
         */
        public final int[] getFrame(final int index) {
            if (img == null || index < prevIndex) { // (Re)Init
                img = new int[width * height];
                prevImg = new int[img.length];
